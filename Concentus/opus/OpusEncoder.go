...
func (enc *OpusEncoder) PrintAllFields() {
	...
	fmt.Printf("enc.Celt_Encoder.mode.overlap: %d\r\n", enc.Celt_Encoder.mode.overlap)
	...
	if st.analysis.enabled {
		...
		if analysis_info.valid != 0 {
			...
			if st.signal_type == OPUS_SIGNAL_AUTO {
				st.voice_ratio = int(math.Floor(.5 + 100*(1-float64(analysis_info.music_prob))))
			}
	...
	if st.mode != MODE_CELT_ONLY {
		...
		if st.energy_masking != nil && st.use_vbr != 0 && st.lfe == 0 {
			...
			for c = 0; c < st.channels; c++ {
				for i = 0; i < end; i++ {
					var mask int
					mask = MAX16Int(MIN16Int(st.energy_masking[21*c+i], int(math.Floor(0.5+(.5)*((1)<<(10))))), -int(math.Floor(0.5+(2.0)*((1)<<(10)))))
			...
			masking_depth += int(int16(math.Floor(0.5 + (.2)*float64((1)<<(10))))) /*Inlines.QCONST16(.2f, 10)*/
...
func (st *OpusEncoder) opus_encode_native(pcm []int16, pcm_ptr, frame_size int, data []byte, data_ptr, out_data_bytes, lsb_depth int, analysis_pcm []int16, analysis_pcm_ptr, analysis_size, c1, c2, analysis_channels, float_api int) int {
	...
	/* Probability of voice in Q7 */
	var equiv_rate = 0
	var delay_compensation = 0
	var frame_rate = 0
	var max_rate = 0
	/* Max bitrate we're allowed to use */
	var curr_bandwidth = 0
	var HB_gain = 0
	var max_data_bytes = 0
	/* Max number of bytes we're allowed to use */
	var total_buffer = 0
	var stereo_width = 0
	var celt_mode *CeltMode
	...
	if st.analysis.enabled {
		...
		if analysis_info.valid != 0 {
			...
			if st.signal_type == OPUS_SIGNAL_AUTO {
				st.voice_ratio = int(math.Floor(0.5 + 100*float64(1-analysis_info.music_prob)))
			}
		}
	}
	...
	if st.application == OPUS_APPLICATION_RESTRICTED_LOWDELAY {
		...
		mode_voice := MULT16_32_Q15Int(CeltConstants.Q15ONE-stereo_width, OpusTables.MODE_THRESHOLDS[0][0]) + MULT16_32_Q15Int(stereo_width, OpusTables.MODE_THRESHOLDS[1][0])
		mode_music := MULT16_32_Q15Int(CeltConstants.Q15ONE-stereo_width, OpusTables.MODE_THRESHOLDS[1][1]) + MULT16_32_Q15Int(stereo_width, OpusTables.MODE_THRESHOLDS[1][1])
		threshold := mode_music + ((voice_est * voice_est * (mode_voice - mode_music)) >> 14)
		if st.application == OPUS_APPLICATION_VOIP {
			threshold += 8000
		}
