...
func silk_quant_LTP_gains(
	...
	for k = 0; k < 3; k++ {
		...
		gain_safety := int(math.Floor(0.4*float64(int64(1)<<(7)) + 0.5))
		...
		for j = 0; j < nb_subfr; j++ {
			max_gain_Q7 = silk_log2lin((int(float64(TuningParameters.MAX_SUM_LOG_GAIN_DB/6.0)*float64(int64(1)<<(7))+0.5)-sum_log_gain_tmp_Q7)+
				int(math.Floor(7*float64(int64(1)<<(7))+0.5))) - gain_safety
			...
			sum_log_gain_tmp_Q7 = silk_max(0, sum_log_gain_tmp_Q7+
				silk_lin2log(gain_safety+gain_Q7)-(int(math.Floor(7*float64(int64(1)<<(7))+0.5))))
