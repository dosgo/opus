...
func silk_process_gains(
	...
	if psEnc.indices.signalType == TYPE_VOICED {
		s_Q16 = 0 - silk_sigm_Q15(silk_RSHIFT_ROUND(psEncCtrl.LTPredCodGain_Q7-int(math.Round((12.0)*(1<<(7))+0.5)), 4))
	...
	InvMaxSqrVal_Q16 = silk_DIV32_16(silk_log2lin(
		silk_SMULWB(int(math.Round((21+16/0.33)*(1<<(7))+0.5))-psEnc.SNR_dB_Q7, int(math.Round(0.33)*(1<<(16))+0.5))), psEnc.subfr_length)
	...
	if psEnc.indices.signalType == TYPE_VOICED {
		if psEncCtrl.LTPredCodGain_Q7+silk_RSHIFT(psEnc.input_tilt_Q15, 8) > int(math.Round((1.0)*(1<<(7))+0.5)) {
