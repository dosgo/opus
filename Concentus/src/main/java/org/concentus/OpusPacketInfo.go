...
func GetBandwidth(packet []byte, packet_offset int) OpusBandwidth {
    var bandwidth OpusBandwidth
    if (packet[packet_offset] & 0x80) != 0 {
        bw := OpusBandwidthMediumband + OpusBandwidth((packet[packet_offset]>>5)&0x3)
        if bw == OpusBandwidthMediumband {
        ...
        bandwidth = OpusBandwidthNarrowband + OpusBandwidth((packet[packet_offset]>>5)&0x3)
    }
    return bandwidth
}
