...
func silk_decode_indices(psDec *SilkChannelDecoder, psRangeDec EntropyCoder, FrameIndex int, decode_LBRR int, condCoding int) {
    ...
    psDec.indices.NLSFIndices[0] = int8(psRangeDec.dec_icdf(psDec.psNLSF_CB.CB1_iCDF, (int(psDec.indices.signalType)>>1*psDec.psNLSF_CB.nVectors, 8))
    NLSF.Silk_NLSF_unpack(ec_ix, pred_Q8, psDec.psNLSF_CB, int(psDec.indices.NLSFIndices[0]))
    ...
    if psDec.nb_subfr == SilkConstants.MAX_NB_SUBFR {
        psDec.indices.NLSFInterpCoef_Q2 = int8(psRangeDec.dec_icdf(SilkTables.Silk_NLSF_interpolation_factor_iCDF, 8))
    } else {
    ...
    if psDec.indices.signalType == SilkConstants.TYPE_VOICED {
        ...
        psDec.ec_prevLagIndex = int(psDec.indices.lagIndex)
