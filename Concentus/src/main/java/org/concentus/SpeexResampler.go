...
func PSHR32(a int32, shift int) int16 {
	if shift > 0 {
		round := int32(1 << (shift - 1))
		return int16((a + round) >> uint(shift))
	}
	return int16(a)
}
...
func QCONST16(x float64, bits int) int16 {
	return int16(x * float64(1<<bits))
}

func EXTRACT16(x int32) int16 {
	return int16(x)
}

func EXTEND32(x int16) int32 {
	return int32(x)
}
