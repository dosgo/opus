...
func (this *OpusMSDecoder) opus_multistream_decode_native(data []byte, data_ptr int, len int, pcm []int16, pcm_ptr int, frame_size int, decode_fec int, soft_clip int) int {
    ...
    for s := 0; s < this.layout.nb_streams; s++ {
        ...
        if s < this.layout.nb_coupled_streams {
            prev := -1
            for {
                chan := OpusMultistream_get_left_channel(&this.layout, s, prev)
                if chan == -1 {
                    break
                }
            ...
            prev = -1
            for {
                chan := OpusMultistream_get_right_channel(&this.layout, s, prev)
                if chan == -1 {
                    break
                }
            ...
            prev := -1
            for {
                chan := OpusMultistream_get_mono_channel(&this.layout, s, prev)
                if chan == -1 {
                    break
                }
