...
const (
	...
	PE_MIN_LAG_MS           = 2
	PE_MAX_LAG_MS           = 18
	PE_LTP_MEM_LENGTH_MS    = 4
	PE_NB_STAGE3_LAGS       = 5
	PE_D_SRCH_LENGTH        = 24
	PE_NB_CBKS_STAGE2_EXT   = 34
	PE_NB_CBKS_STAGE2       = 20
	PE_NB_CBKS_STAGE2_10MS  = 12
	PE_MAX_NB_SUBFR         = 4
...
type BoxedValueByte struct{ Val byte }
...
func silk_RSHIFT_uint(a uint32, shift uint) uint32 {
	return a >> shift
}
...
func silk_SMLAWB(a, b, c int) int {
	return a + int(int16(b)*int16(c))
}

func silk_DIV32_varQ(a, b int32, Q int) int32 {
	return int32((int64(a) << uint(Q)) / int64(b))
}
