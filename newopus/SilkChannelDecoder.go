...
type SilkChannelDecoder struct {
    prev_gain_Q16           int
    exc_Q14                 [SilkConstants_MAX_FRAME_LENGTH]int
    sLPC_Q14_buf            [SilkConstants_MAX_LPC_ORDER]int
    outBuf                  [SilkConstants_MAX_FRAME_LENGTH + 2*SilkConstants_MAX_SUB_FRAME_LENGTH]int16
    lagPrev                 int
    LastGainIndex           int8
    fs_kHz                  int
    fs_API_hz               int
    ...
    ltp_mem_length          int
    LPC_order               int
    prevNLSF_Q15            [SilkConstants_MAX_LPC_ORDER]int16
    ...
    VAD_flags               [SilkConstants_MAX_FRAMES_PER_PACKET]int
    LBRR_flag               int
    LBRR_flags              [SilkConstants_MAX_FRAMES_PER_PACKET]int
...
func (d *SilkChannelDecoder) Reset() {
    d.prev_gain_Q16 = 0
    d.exc_Q14 = [SilkConstants_MAX_FRAME_LENGTH]int{}
    d.sLPC_Q14_buf = [SilkConstants_MAX_LPC_ORDER]int{}
    d.outBuf = [SilkConstants_MAX_FRAME_LENGTH + 2*SilkConstants_MAX_SUB_FRAME_LENGTH]int16{}
    d.lagPrev = 0
    d.LastGainIndex = 0
    d.fs_kHz = 0
    d.fs_API_hz = 0
    ...
    d.ltp_mem_length = 0
    d.LPC_order = 0
    d.prevNLSF_Q15 = [SilkConstants_MAX_LPC_ORDER]int16{}
    ...
    d.ec_prevLagIndex = 0
    d.VAD_flags = [SilkConstants_MAX_FRAMES_PER_PACKET]int{}
    d.LBRR_flag = 0
    d.LBRR_flags = [SilkConstants_MAX_FRAMES_PER_PACKET]int{}
...
func (d *SilkChannelDecoder) silk_decoder_set_fs(fs_kHz, fs_API_Hz int) int {
    ...
    if d.fs_kHz != fs_kHz || frame_length != d.frame_length {
        if fs_kHz == 8 {
            if d.nb_subfr == SilkConstants_MAX_NB_SUBFR {
                d.pitch_contour_iCDF = SilkTables_silk_pitch_contour_NB_iCDF
            } else {
                d.pitch_contour_iCDF = SilkTables_silk_pitch_contour_10_ms_NB_iCDF
            }
        } else if d.nb_subfr == SilkConstants_MAX_NB_SUBFR {
            d.pitch_contour_iCDF = SilkTables_silk_pitch_contour_iCDF
        } else {
            d.pitch_contour_iCDF = SilkTables_silk_pitch_contour_10_ms_iCDF
        }
        if d.fs_kHz != fs_kHz {
            d.ltp_mem_length = Inlines_silk_SMULBB(SilkConstants_LTP_MEM_LENGTH_MS, fs_kHz)
            if fs_kHz == 8 || fs_kHz == 12 {
                d.LPC_order = SilkConstants_MIN_LPC_ORDER
                d.psNLSF_CB = SilkTables_silk_NLSF_CB_NB_MB
            } else {
                d.LPC_order = SilkConstants_MAX_LPC_ORDER
            ...
            d.lagPrev = 100
            d.LastGainIndex = 10
            d.prevSignalType = SilkConstants_TYPE_NO_VOICE_ACTIVITY
            d.outBuf = [SilkConstants_MAX_FRAME_LENGTH + 2*SilkConstants_MAX_SUB_FRAME_LENGTH]int16{}
            d.sLPC_Q14_buf = [SilkConstants_MAX_LPC_ORDER]int{}
        }
        d.fs_kHz = fs_kHz
    ...
    Inlines_OpusAssert(d.frame_length > 0 && d.frame_length <= SilkConstants_MAX_FRAME_LENGTH)
    return ret
}
...
func (d *SilkChannelDecoder) silk_decode_frame(psRangeDec *EntropyCoder, pOut []int16, pOut_ptr int, pN *int, lostFlag, condCoding int) int {
    ...
    Inlines_OpusAssert(L > 0 && L <= SilkConstants_MAX_FRAME_LENGTH)
    ret := 0

    if lostFlag == DecoderAPIFlag_FLAG_DECODE_NORMAL || (lostFlag == DecoderAPIFlag_FLAG_DECODE_LBRR && d.LBRR_flags[d.nFramesDecoded] == 1) {
        pulses := make([]int16, (L+SilkConstants_SHELL_CODEC_FRAME_LENGTH-1)&^(SilkConstants_SHELL_CODEC_FRAME_LENGTH-1))
