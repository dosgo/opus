...
func silk_noise_shape_analysis(psEnc *SilkChannelEncoder, psEncCtrl *SilkEncoderControl, pitch_res []int16, pitch_res_ptr int, x []int16, x_ptr int) {
	...
	if psEnc.useCBR == 0 {
		...
		SNR_adj_dB_Q7 = silk_SMLAWB(SNR_adj_dB_Q7,
			(int32(-BG_SNR_DECR_dB<<7)>>(4+1))*b_Q8,
	...
	if psEnc.indices.signalType == TYPE_VOICED {
		...
		psEncCtrl.sparseness_Q8 = silk_sigm_Q15(silk_SMULWB(energy_variation_Q7-(5<<7), int32(0.1*float64(1<<16)))) >> 7
		if psEncCtrl.sparseness_Q8 > int32(SPARSENESS_THRESHOLD_QNT_OFFSET<<8) {
			psEnc.indices.quantOffsetType = 0
		} else {
			psEnc.indices.quantOffsetType = 1
		}
		SNR_adj_dB_Q7 = silk_SMLAWB(SNR_adj_dB_Q7, int32(SPARSE_SNR_INCR_dB<<15), psEncCtrl.sparseness_Q8-(1<<7))
	}
	...
	if psEnc.warping_Q16 > 0 {
		warping_Q16 = silk_SMLAWB(psEnc.warping_Q16, int32(psEncCtrl.coding_quality_Q14), int32(0.01*float64(1<<18)))
	} else {
		warping_Q16 = 0
	}
	...
	for k = 0; k < psEnc.nb_subfr; k++ {
		...
		auto_corr[0] = silk_ADD32(auto_corr[0], silk_max_32(silk_SMULWB(auto_corr[0]>>4, int32(SHAPE_WHITE_NOISE_FRACTION<<20)), 1))
		...
		if psEnc.warping_Q16 > 0 {
			...
			if silk_SMULWW(psEncCtrl.Gains_Q16[k]>>1, gain_mult_Q16) >= (int32(1) << 30) {
		...
		pre_nrg_Q30 = silk_SMULWB(pre_nrg_Q30, int32(0.7*float64(1<<15))) << 1
		psEncCtrl.GainsPre_Q14[k] = int32(0.3*float64(1<<14)) + silk_DIV32_varQ(pre_nrg_Q30, nrg, 14)
		limit_warped_coefs(AR2_Q24, AR1_Q24, warping_Q16, int32(3.999*float64(1<<24)), psEnc.shapingLPCOrder)
	...
	gain_mult_Q16 = silk_log2lin(-silk_SMLAWB(-(16 << 7), SNR_adj_dB_Q7, int32(0.16*float64(1<<16))))
	gain_add_Q16 = silk_log2lin(silk_SMLAWB(16<<7, int32(MIN_QGAIN_DB<<7), int32(0.16*float64(1<<16))))
	...
	gain_mult_Q16 = (1 << 16) + ((int32(INPUT_TILT<<26) + psEncCtrl.coding_quality_Q14*int32(HIGH_RATE_INPUT_TILT<<12)) >> 10)
	for k = 0; k < psEnc.nb_subfr; k++ {
	...
	if psEnc.indices.signalType == TYPE_VOICED {
		...
		Tilt_Q16 = -int32(HP_NOISE_COEF << 16)
	}

	HarmBoost_Q16 = silk_SMULWB(silk_SMULWB((1<<17)-int32(psEncCtrl.coding_quality_Q14<<3), psEnc.LTPCorr_Q15), int32(LOW_RATE_HARMONIC_BOOST<<16))
	HarmBoost_Q16 = silk_SMLAWB(HarmBoost_Q16, (1<<16)-int32(psEncCtrl.input_quality_Q14<<2), int32(LOW_INPUT_QUALITY_HARMONIC_BOOST<<16))
	if USE_HARM_SHAPING != 0 && psEnc.indices.signalType == TYPE_VOICED {
		HarmShapeGain_Q16 = silk_SMLAWB(int32(HARMONIC_SHAPING<<16), (1<<16)-silk_SMULWB((1<<18)-int32(psEncCtrl.coding_quality_Q14<<4), psEncCtrl.input_quality_Q14), int32(HIGH_RATE_OR_LOW_QUALITY_HARMONIC_SHAPING<<16))
